...
type MerkleProof struct {
	...
	AccountProof []string       `json:"accountProof"` // null for pending tx
	Balance      string         `json:"balance"`      // Parity only; null when not creating contract
...
type MerkleProofResponse struct {
	ResponseBase
	Result MerkleProof `json:"result"`
}
...
func (client *EthereumClient) Eth_getProof(addr string, slot string) (MerkleProof, error) {
	reqBody := JSONRPCRequest{
		JSONRPC: "2.0",
		ID:      1,
		Method:  "eth_getProof",
		Params:  []interface{}{addr, []interface{}{slot}, "latest"},
	}
	res2, err := client.issueRequest(&reqBody)
	if err != nil {
	...
	var clientResp MerkleProofResponse
	err = json.Unmarshal(res2, &clientResp)
	if err != nil {
		return MerkleProof{}, err
	}
...
func (client *EthereumClient) Eth_getStorageAt(addr string, slot string) (string, error) {
	reqBody := JSONRPCRequest{
		JSONRPC: "2.0",
		ID:      1,
		Method:  "eth_getStorageAt",
		Params:  []interface{}{addr, slot, "latest"},
	}
	res2, err := client.issueRequest(&reqBody)
	if err != nil {
		return "", err
